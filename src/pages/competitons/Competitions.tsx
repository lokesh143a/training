...
const Competitions = () => {
  ...
  const transformedTableData: Array<Record<string, React.ReactNode>> =
    filteredData.map((item) => ({
      ...
      competitionName: (
        <p
          onClick={() => navigate("/competitions-teams")}
      ...
      status:
        item.status === "Complete" ? (
          <span className="inline-block text-center text-[8px] md:text-[14px] rounded-full text-[#22C55E] bg-green-50 w-full md:w-[97px] h-[25px] font-semibold">
            {item.status}
          </span>
        ) : (
